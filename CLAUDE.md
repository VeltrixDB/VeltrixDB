...
49. **The ordered index is touched only when a key becomes live.** `shardedIndex.put`, `replayPut` and the atomic-op install call `ordered.Insert` only when the replaced entry was absent or a tombstone — the same condition as `keyCount`. A live key is already in the skiplist (key live in entries ⇔ key in ordered), and calling Insert on every overwrite cost an O(log N) search, ~20 cache misses at 1M keys (480 → 1351 ns per put), serially 1024 times per MultiPut: skipping it took batch writes 1.45M → 3.54M keys/s, P99 9.6 → 4.2 ms. A 64-way partitioned skiplist was tried first (the profile looked like lock contention) and measured no gain — the cost was the search, not the locks. Never reintroduce an unconditional Insert on the put path.

50. **`netfront/` (C++ front-end, `--net=cpp|uring|poll`) — per-connection order and buffer lifetime are the invariants.** (a) A connection with a write in flight is not parsed further until the write is answered; that is what gives pipelined PUT-then-GET read-your-writes (`TestNetfront_PipelineOrder` fails without it). (b) `vxnf_req` pointers reference the connection's input buffer and must stay valid until `vxnfExec` returns — grow/compact/free only in `Loop::post()`, and never while a recv is armed. (c) Output is double-buffered; the kernel only ever sees `out_inflight`. (d) Slots are released only with nothing armed, bumping `gen`; `vxnf_respond` from any goroutine is safe until `vxnf_free`, and `Server.Close` waits for in-flight write goroutines before freeing. (e) Only the loop thread touches its `conns` — `vxnf_stop` just signals. Scope: standalone mode, no `--auth-config`; the server refuses otherwise. The io_uring backend compiles and runs only on Linux; `.github/workflows/net-bench.yml` tests it (`VXNF_TEST_BACKEND=uring`) and compares `--net=go|uring|poll`. On macOS only the poll backend runs, with a single loop, and loopback caps round trips at ~60K/s for C++ and Go alike — do not judge network changes from a Mac. (f) **A read that needs disk never runs on a loop thread.** GET/MGET probe with `StorageEngine.GetNoIO`; a key that needs a VLog read is finished by `GetAfterNoIO` in a goroutine, and `vxnf_defer` blocks the connection like a write so later batches cannot overtake it. Within the batch, every later request of that connection joins the same ordered chain. `TestNetfront_DeferredReadBlocksLaterBatches` fails without `vxnf_defer`, and `TestNetfront_DeferredReadNoHeadOfLine` fails if disk reads go back inline. `VELTRIXDB_NET_DEFER_READS=0` is the A/B switch. (g) `Server.Close` snapshots the C++ histograms before `vxnf_free`; `LatencyReport` after Close must never touch `s.c`.

51. **The io_uring VLog write bridge is opt-in: `VELTRIXDB_URING_BRIDGE=off` (default) | `on` | `sqpoll`.** It used to start unconditionally on every Linux cgo engine with SQPOLL on. A VLog batch is already ONE pwrite plus one fdatasync (invariant 42), so the bridge can save at most one syscall per batch, and SQPOLL spends a kernel thread busy-polling to save it. On the 4-CPU CI runner (tmpfs, 2026-09-25), the C++ storage layer — this bridge with SQPOLL, the batch engine and the native index, all on — measured batch writes at 1.12M keys/s with P99 ~19 ms. With all three off it was 1.77M keys/s and ~12.7 ms. That run changed all three at once, so the bench now bisects them (`ENGINES="native bridge sqpoll map nocgo"`). Do not re-enable by default without a per-part measurement on the target hardware. CI `node-6-cpp` runs the storage suite with `on` (not `sqpoll`: many short-lived SQPOLL engines starve a runner). `VELTRIXDB_DISABLE_CGO_ENGINE=1` still forces it off, and also selects the map index.
